...
BAOSTOCK_FETCH_ADJUSTFLAG = "3"  # 1=后复权，2=前复权，3=不复权
CORPORATE_ACTION_FACTOR_TOLERANCE = 0.0005  # 0.05%
...
# ========================== 价格复权修复 ==========================
def rebuild_forward_adjusted_ohlc(
    df: pd.DataFrame,
    action_tolerance: float = CORPORATE_ACTION_FACTOR_TOLERANCE,
) -> pd.DataFrame:
    """
    依据每日 preclose 构造连续的前复权 OHLC。

    Baostock 对部分 ETF 的拆分、份额折算数据会出现以下情况：
      - 昨日 close 仍是折算前价格；
      - 今日 close 已是折算后价格；
      - pctChg 却按交易所的除权参考价计算。

    这种输入会让 MACD/KDJ 把份额折算误判为暴跌。前复权的单次因子为：

        step_factor = 当日 preclose / 前一交易日 raw_close

    从最新交易日向历史方向累计该因子，再同时乘到 open/high/low/close。
    最新交易日因子固定为 1，因此最终价格尺度与行情软件“前复权”一致。

    原始行情保留在 raw_open/raw_high/raw_low/raw_close/raw_preclose 中，
    便于核对复权事件。
    """
    required = {"date", "open", "high", "low", "close"}
    missing = required.difference(df.columns)
    if missing:
        raise ValueError(f"前复权修复缺少字段：{sorted(missing)}")
    ...
    if "preclose" in result.columns:
        result["preclose"] = pd.to_numeric(result["preclose"], errors="coerce")
        result["raw_preclose"] = result["preclose"]
    else:
        result["preclose"] = pd.NA
        result["raw_preclose"] = pd.NA

    if "pctChg" in result.columns:
        result["pctChg"] = pd.to_numeric(result["pctChg"], errors="coerce")

    row_count = len(result)
    factors = [1.0] * row_count
    event_flags = [0] * row_count
    step_factors = [1.0] * row_count
    factor_sources = [""] * row_count

    current_factor = 1.0
    raw_close = result["raw_close"]
    raw_preclose = result["raw_preclose"]
    pct_chg = result["pctChg"] if "pctChg" in result.columns else None

    # 最新一行保持原价；逐日向前累计除权/折算因子。
    for current_index in range(row_count - 1, 0, -1):
        previous_close = raw_close.iloc[current_index - 1]
        step_factor = 1.0
        source = "none"

        if pd.notna(previous_close) and float(previous_close) > 0:
            current_preclose = raw_preclose.iloc[current_index]

            if pd.notna(current_preclose) and float(current_preclose) > 0:
                step_factor = float(current_preclose) / float(previous_close)
                source = "preclose"
            elif pct_chg is not None:
                current_close = raw_close.iloc[current_index]
                current_pct = pct_chg.iloc[current_index]
                if (
                    pd.notna(current_close)
                    and float(current_close) > 0
                    and pd.notna(current_pct)
                    and float(current_pct) > -100
                ):
                    implied_preclose = float(current_close) / (
                        1.0 + float(current_pct) / 100.0
                    )
                    step_factor = implied_preclose / float(previous_close)
                    source = "pctChg_fallback"

        valid_factor = math.isfinite(step_factor) and step_factor > 0
        if not valid_factor:
            step_factor = 1.0
            source = "invalid_ignored"

        # 正常交易日 preclose 与上一日 close 相同，因子应为 1。
        # 只累计超过容差的变化，避免小数位舍入造成因子逐日漂移。
        if abs(step_factor - 1.0) > action_tolerance:
            current_factor *= step_factor
            event_flags[current_index] = 1
            step_factors[current_index] = step_factor
            factor_sources[current_index] = source

        factors[current_index - 1] = current_factor

    result["price_adjust_factor"] = pd.Series(
        factors, index=result.index, dtype="float64"
    )
    result["adjustment_event_flag"] = pd.Series(
        event_flags, index=result.index, dtype="int64"
    )
    result["adjustment_step_factor"] = pd.Series(
        step_factors, index=result.index, dtype="float64"
    )
    result["adjustment_factor_source"] = factor_sources

    for column in ("open", "high", "low", "close"):
        result[column] = result[f"raw_{column}"] * result["price_adjust_factor"]

    # preclose 也转换到相同前复权尺度，便于导出后人工检查。
    result["preclose"] = (
        pd.to_numeric(result["raw_preclose"], errors="coerce")
        * result["price_adjust_factor"]
    )

    result["raw_return_pct"] = (
        result["raw_close"].pct_change(fill_method=None) * 100.0
    )
    result["adjusted_return_pct"] = (
        result["close"].pct_change(fill_method=None) * 100.0
    )
    if "pctChg" in result.columns:
        result["raw_return_gap_to_pctChg"] = (
            result["raw_return_pct"] - result["pctChg"]
        )
        result["adjusted_return_gap_to_pctChg"] = (
            result["adjusted_return_pct"] - result["pctChg"]
        )

    events = result.loc[result["adjustment_event_flag"] == 1]
    if events.empty:
        print("  未检测到拆分、份额折算或除权因子变化")
    else:
        print(f"  检测到 {len(events)} 个复权因子变化：")
        for _, row in events.iterrows():
            print(
                "    "
                f"{row['date'].strftime('%Y-%m-%d')}｜"
                f"单次因子={float(row['adjustment_step_factor']):.9f}｜"
                f"来源={row['adjustment_factor_source']}｜"
                f"原始前收={format_number(row['raw_preclose'], 6)}"
            )
...
def calculate_macd(df: pd.DataFrame, prefix: str = "") -> pd.DataFrame:
    """按行情软件常用 EMA 递推公式计算 MACD(10,22,9)。"""
...
def calculate_kdj(df: pd.DataFrame, prefix: str = "") -> pd.DataFrame:
    """
    按国内行情软件常用公式计算 KDJ(9,3,3)。

    与直接调用 ewm 不同，这里显式使用 K0=D0=50：
    ...
    low_list = low.rolling(KDJ_N_PERIOD, min_periods=1).min()
    high_list = high.rolling(KDJ_N_PERIOD, min_periods=1).max()
    price_range = high_list - low_list

    rsv = (close - low_list) / price_range.replace(0, pd.NA) * 100.0
    rsv = pd.to_numeric(rsv, errors="coerce").fillna(KDJ_INITIAL_VALUE)

    k_values: List[float] = []
    d_values: List[float] = []
    previous_k = KDJ_INITIAL_VALUE
    previous_d = KDJ_INITIAL_VALUE

    k_weight = 1.0 / float(KDJ_K_SMOOTH)
    d_weight = 1.0 / float(KDJ_D_SMOOTH)

    for current_rsv in rsv.tolist():
        current_k = (
            (1.0 - k_weight) * previous_k
            + k_weight * float(current_rsv)
        )
        current_d = (
            (1.0 - d_weight) * previous_d
            + d_weight * current_k
        )
    ...
    k = pd.Series(k_values, index=df.index, dtype="float64")
    d = pd.Series(d_values, index=df.index, dtype="float64")
    j = 3.0 * k - 2.0 * d
...
def get_etf_full_data(code: str, configured_name: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        output_start_timestamp = pd.Timestamp(GLOBAL_START_DATE).normalize()
        calculation_start_timestamp = (
            output_start_timestamp
            - pd.Timedelta(days=INDICATOR_WARMUP_CALENDAR_DAYS)
        )

        if ipo_date:
            ipo_timestamp = pd.Timestamp(ipo_date).normalize()
            calculation_start_timestamp = max(
                calculation_start_timestamp,
                ipo_timestamp,
            )
            print(
                f"上市日期：{ipo_date}，指标计算起始日期："
                f"{calculation_start_timestamp.strftime('%Y-%m-%d')}"
            )
        else:
            print(
                "上市日期：未获取；指标计算从输出起始日前 "
                f"{INDICATOR_WARMUP_CALENDAR_DAYS} 个自然日开始"
            )

        start_date = calculation_start_timestamp.strftime("%Y-%m-%d")
        print(f"最终输出起始日期：{GLOBAL_START_DATE}")
        ...
        rs = bs.query_history_k_data_plus(
            ...
            end_date=END_DATE,
            frequency="d",
            adjustflag=BAOSTOCK_FETCH_ADJUSTFLAG,
        )
        ...
        print("正在依据 preclose 重建连续前复权 OHLC...")
        df = rebuild_forward_adjusted_ohlc(df)
        ...
        # 预热数据只参与指标递推，不写入最终明细。
        df = df.loc[df["date"] >= output_start_timestamp].reset_index(drop=True)
        if df.empty:
            print(f"输出起始日期 {GLOBAL_START_DATE} 之后没有有效数据")
            return None
        ...
        print(
            "日线 MACD 数值："
            f"DIF={latest['day_dif']:.6f}，"
            f"DEA={latest['day_dea']:.6f}，"
            f"MACD={latest['day_macd']:.6f}"
        )
        print(
            "日线 KDJ 数值："
            f"K={latest['day_k']:.6f}，"
