...
def add_weekly_indicators(daily_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 周线当前状态在“唯一周线数据”上计算，不在重复映射后的日线上计算。
    weekly_df = add_current_cross_status(
        weekly_df, "week_dif", "week_dea", "week_macd_status"
    )
    weekly_df = add_current_cross_status(
        weekly_df, "week_k", "week_d", "week_kdj_status"
    )
...
def get_etf_full_data(code: str, configured_name: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        df = add_current_cross_status(df, "day_dif", "day_dea", "day_macd_status")
        df = add_current_cross_status(df, "day_k", "day_d", "day_kdj_status")

        # 日 MACD：使用最近两个交易日的 DIF-DEA 差值预测下一交易日。
        ...
        print(f"日线 MACD 当前：{status_text(latest['day_macd_status'])}")
        print(f"周线 KDJ 当前：{status_text(latest['week_kdj_status'])}")
        print(f"周线 MACD 当前：{status_text(latest['week_macd_status'])}")
