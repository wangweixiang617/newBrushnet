...
def _run_main(program_start_perf: float) -> None:
    ...
    # tqdm 是科研/数据处理中常用的进度条工具。
    # 进度条显示：当前标的、完成数量、已用时间、预计剩余时间和处理速度。
    progress_bar_format = (
    ...
    with tqdm(
        ...
        for index, item in enumerate(ETF_CONFIG, start=1):
            ...
            try:
                ...
                progress_bar.set_description_str("当前：配置错误")
                progress_bar.update(1)
                progress_bar.set_postfix_str(
                    f"失败 | 本个 {format_elapsed_time(item_elapsed)} | "
                    f"累计 {format_elapsed_time(total_elapsed)}"
                )
                tqdm.write(
                    f"✗ 跑完第 {index}/{total_count} 个：配置错误｜"
                    f"本个耗时 {format_elapsed_time(item_elapsed)}｜"
                    f"累计耗时 {format_elapsed_time(total_elapsed)}｜{exc}"
                )
                continue

            # 当前正在处理的 ETF 名称和代码直接显示在进度条上。
            progress_bar.set_description_str(
                f"当前：{configured_name}（{code}）"
            )
            progress_bar.set_postfix_str("正在获取和计算")
        ...
        progress_bar.set_description_str("ETF 处理完成")
        progress_bar.set_postfix_str(
            f"成功 {success_count} | 失败 {failed_count} | "
            f"累计 {format_elapsed_time(time.perf_counter() - program_start_perf)}"
        )

    print("\n" + "=" * 72)
    print(
        f"全部标的处理完成：成功 {success_count}/{total_count} 个，"
        f"失败 {failed_count}/{total_count} 个"
    )
    print(
        f"行情获取与指标计算累计耗时："
    ...
    print(
        f"红色预警说明：Excel 汇总 Sheet 中持续时间为 1 的指标已标红；"
        f"控制台使用 {NEW_CROSS_MARKER} 标记。"
    )
